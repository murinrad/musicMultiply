...
public class NtpServer extends AbstractProtocolService {
     ...
     * The default IP port.
     */
    private int ipPort = NTPSettings.NTP_PORT;
     ...
     * <p/>
     * Note that we don't have any filter in the chain, everything is done in
     * the handler.
     *
    ...
    public void start() throws IOException {
        ...
        ntpChain.addLast("codec", new ProtocolCodecFilter(
                NtpProtocolCodecFactory.getInstance()));

        if ((transports == null) || (transports.size() == 0)) {
            // Default to UDP with port 123
            // We have to create a DatagramAcceptor
            UdpTransport transport = new UdpTransport(ipPort);
            ...
            for (Transport transport : transports) {
                ...
                if (transport instanceof UdpTransport) {
                    ...
                    ((SocketAcceptor) acceptor).getSessionConfig()
                            .setTcpNoDelay(true);
                }
    ...
    public String toString() {
        ...
        sb.append("NTPServer[").append(getServiceName())
                .append("], listening on :").append('\n');
